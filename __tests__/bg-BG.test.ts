...
import bgBg, { ToNumbers as LocaleToNumbers } from '../src/locales/bg-BG';
...
describe('Test Locale', () => {
  test(`Locale Class: ${localeCode}`, () => {
    expect(toNumbers.getLocaleClass()).toBe(bgBg);
  });
...
const testIntegers: [string, number][] = [
  ...
  ['Четири Хиляди', 4000],
  ['Пет Хиляда', 5000],
  ['Четири Хиляди Шестстотин Осемдесет', 4680],
  ['Десет Хиляда', 10000],
  ['Шестдесет Три Хиляди Осемстотин Деветдесет Две', 63892],
  ['Сто Хиляди', 100000],
  ['Милион', 1000000],
  ['Две Милиона', 2000000],
  ['Пет Милион', 5000000],
  ['Две Милиона Седемстотин Четиридесет Едно Хиляди Тридесет Четири', 2741034],
  ['Милиард', 1000000000],
  ['Две Милиарда', 2000000000],
  ['Пет Милиард', 5000000000],
];
...
const testFloats: [string, number][] = [
  ['Нула', 0],
  ['Нула Цяло Нула Четири', 0.04],
  ['Нула Цяло Четири', 0.4],
  ['Нула Цяло Шестдесет Три', 0.63],
  ['Тридесет Седем Цяло Нула Шест', 37.06],
  ['Тридесет Седем Цяло Шестдесет Осем', 37.68],
];
...
describe('Test Ordinal Parse Result', () => {
  test('parse returns isOrdinal true for ordinal input', () => {
    const result = toNumbers.parse('Първи');
  ...
  test('parse returns isOrdinal undefined for cardinal input', () => {
    const result = toNumbers.parse('Едно');
    expect(result.value).toBe(1);
...
const testFractionDecimals: [string, number][] = [
  ['Нула Цяло Едно Десетина', 0.1],
  ['Нула Цяло Три Десетини', 0.3],
  ['Нула Цяло Едно Стотина', 0.01],
  ['Нула Цяло Три Стотини', 0.03],
  ['Нула Цяло Едно Хилядна', 0.001],
  ['Нула Цяло Три Хиляди', 0.003],
  ['Нула Цяло Едно Десетохилядна', 0.0001],
  ['Нула Цяло Три Десетохиляди', 0.0003],
  ['Нула Цяло Едно Стохилядна', 0.00001],
  ['Нула Цяло Три Стохиляди', 0.00003],
  ['Нула Цяло Едно Милионна', 0.000001],
  ['Нула Цяло Три Милиони', 0.000003],
];
...
const testGenderedForms: [string, number][] = [['Една', 1]];
