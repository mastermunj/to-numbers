...
describe('Test Ordinal Parse Result', () => {
  test('parse returns isOrdinal true for ordinal input', () => {
    const result = toNumbers.parse('మొదటి');
  ...
  test('parse returns isOrdinal undefined for cardinal input', () => {
    const result = toNumbers.parse('ఒకటి');
    expect(result.value).toBe(1);
...
const testFractionDecimals: [string, number][] = [
  ['సున్నా బిందువు ఒకటి దశాంశం', 0.1],
  ['సున్నా బిందువు ఒకటి శతాంశం', 0.01],
  ['సున్నా బిందువు ఒకటి సహస్రాంశం', 0.001],
  ['సున్నా బిందువు ఒకటి దశ-సహస్రాంశం', 0.0001],
  ['సున్నా బిందువు ఒకటి శత-సహస్రాంశం', 0.00001],
  ['సున్నా బిందువు ఒకటి దశ-లక్షాంశం', 0.000001],
];
