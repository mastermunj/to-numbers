...
import caEs, { ToNumbers as LocaleToNumbers } from '../src/locales/ca-ES';
...
describe('Test Locale', () => {
  test(`Locale Class: ${localeCode}`, () => {
    expect(toNumbers.getLocaleClass()).toBe(caEs);
  });
...
const testIntegers: [string, number][] = [
  ...
  ['Mil Cent', 1100],
  ['Dos Mil', 2000],
  ['Quatre Mil Sis-Cents Vuitanta', 4680],
  ['Deu Mil', 10000],
  ['Seixanta Tres Mil Vuit-Cents Noranta Dos', 63892],
  ['Cent Mil', 100000],
  ['Un Milió', 1000000],
  ['Dos Milións', 2000000],
  ['Dos Milións Set-Cents Quaranta Un Mil Trenta Quatre', 2741034],
  ['Mil Milions', 1000000000],
  ['Dos Mil Milions', 2000000000],
];
...
const testFloats: [string, number][] = [
  ['Zero', 0],
  ['Zero Coma Zero Quatre', 0.04],
  ['Zero Coma Quatre', 0.4],
  ['Zero Coma Seixanta Tres', 0.63],
  ['Trenta Set Coma Zero Sis', 37.06],
  ['Trenta Set Coma Seixanta Vuit', 37.68],
];
...
describe('Test Ordinal Parse Result', () => {
  test('parse returns isOrdinal true for ordinal input', () => {
    const result = toNumbers.parse('Primer');
