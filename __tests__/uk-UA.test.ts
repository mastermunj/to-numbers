...
import ukUa, { ToNumbers as LocaleToNumbers } from '../src/locales/uk-UA';
...
describe('Test Locale', () => {
  test(`Locale Class: ${localeCode}`, () => {
    expect(toNumbers.getLocaleClass()).toBe(ukUa);
  });
...
const testIntegers: [string, number][] = [
  ...
  ['Тисяча Сто', 1100],
  ['Два Тисячі', 2000],
  ['Чотири Тисячі Шістсот Вісімдесят', 4680],
  ['Десять Тисяча', 10000],
  ['Сто Тисяч', 100000],
  ['Мільйон', 1000000],
  ['Два Мільйони', 2000000],
  ['Два Мільйони Сімсот Сорок Один Тисяч Тридцять Чотири', 2741034],
  ['Мільярд', 1000000000],
  ['Два Мільярди', 2000000000],
];
...
const testFloats: [string, number][] = [
  ['Нуль', 0],
  ['Нуль Цілих Нуль Чотири', 0.04],
  ['Нуль Цілих Чотири', 0.4],
  ['Нуль Цілих Шістдесят Три', 0.63],
  ['Тридцять Сім Цілих Нуль Шість', 37.06],
  ['Тридцять Сім Цілих Шістдесят Вісім', 37.68],
];
...
describe('Test Ordinal Parse Result', () => {
  test('parse returns isOrdinal true for ordinal input', () => {
    const result = toNumbers.parse('Перший');
...
const testFractionDecimals: [string, number][] = [
  ['Нуль Цілих Один Десята', 0.1],
  ['Нуль Цілих Три Десятих', 0.3],
  ['Нуль Цілих Один Сота', 0.01],
  ['Нуль Цілих Три Сотих', 0.03],
  ['Нуль Цілих Один Тисячна', 0.001],
  ['Нуль Цілих Три Тисячних', 0.003],
  ['Нуль Цілих Один Десятитисячна', 0.0001],
  ['Нуль Цілих Три Десятитисячних', 0.0003],
  ['Нуль Цілих Один Стотисячна', 0.00001],
  ['Нуль Цілих Три Стотисячних', 0.00003],
  ['Нуль Цілих Один Мільйонна', 0.000001],
  ['Нуль Цілих Три Мільйонних', 0.000003],
];
