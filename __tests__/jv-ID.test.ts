...
import jvId, { ToNumbers as LocaleToNumbers } from '../src/locales/jv-ID';
...
describe('Test Locale', () => {
  test(`Locale Class: ${localeCode}`, () => {
    expect(toNumbers.getLocaleClass()).toBe(jvId);
  });
  ...
  config.numberWordsMapping
    ...
    .forEach((entry) => {
      ...
      values.forEach((v: string) => {
        wordToEntry.set(v.toLowerCase(), { number: entry.number as number, value: v });
      });
    });
...
const testIntegers: [string, number][] = [
  ['Nol', 0],
  ['Satus Telung Puluh Pitu', 137],
  ['Pitung Atus', 700],
  ['Sèwu Satus', 1100],
  ['Papat Èwu Nem Atus Wolung Puluh', 4680],
  ['Sewidak Telu Èwu Wolu Atus Sangang Puluh Loro', 63892],
  ['Loro Yuta Pitu Atus Patang Puluh Siji Èwu Telung Puluh Papat', 2741034],
  ['Sangang Milyar Wolu Atus Pitung Puluh Nem Yuta Limang Atus Patang Puluh Telu Èwu Rong Atus Sepuluh', 9876543210],
];
...
const testFloats: [string, number][] = [
  ['Nol', 0],
  ['Nol Koma Papat', 0.4],
  ['Telung Puluh Pitu Koma Nem', 37.6],
];
