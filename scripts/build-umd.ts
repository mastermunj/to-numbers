...
async function buildBundle(
  ...
  // ESM export looks like: export{L as ToNumbers,W as default};
  // We need to find the ToNumbers export and return it
  // Match pattern: SomeVar as ToNumbers
  const exportMatch = code.match(/export\s*\{([^}]+)\}\s*;?\s*$/);
  if (exportMatch) {
    const exports = exportMatch[1];
    // Find "X as ToNumbers" pattern
    const toNumbersMatch = exports.match(/(\w+)\s+as\s+ToNumbers/);
    if (toNumbersMatch) {
      const varName = toNumbersMatch[1];
      // Remove the export statement and add return
      code = code.replace(/export\s*\{[^}]+\}\s*;?\s*$/, `return ${varName};`);
    }
  }

  const wrapped = `"use strict";var ToNumbers=(()=>{${code}})();`;
...
async function main(): Promise<void> {
  ...
  const fullResult = await buildBundle(fullEntry, path.join(DIST_DIR, 'to-numbers.min.js'), true);
  ...
  if (verbose) {
    ...
    console.log('   <script src="https://cdn.jsdelivr.net/npm/to-numbers/dist/umd/to-numbers.min.js"></script>');
    console.log('   <script>');
    console.log("     const tn = new ToNumbers({ localeCode: 'en-IN' });");
    console.log("     console.log(tn.convert('One Thousand Two Hundred Thirty Four'));");
    console.log('   </script>');
    console.log('');
    console.log('   <!-- Single locale (smaller bundle) -->');
    console.log('   <script src="https://cdn.jsdelivr.net/npm/to-numbers/dist/umd/en-IN.min.js"></script>');
    console.log('   <script>');
    console.log('     const tn = new ToNumbers();');
    console.log("     console.log(tn.convert('One Thousand Two Hundred Thirty Four'));");
    console.log('   </script>');
    console.log('');
  }
